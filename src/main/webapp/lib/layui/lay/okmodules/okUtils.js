...
layui.define(["layer"], function (exprots) {
    ...
    var okUtils = {
        ...
        baseUrl: "http://49.235.133.129:7788/bls/api",
        // baseUrl: "http://127.0.0.1:7788/bls/api",
        /**
         * 获取body的总宽度
         */
         ...
         * ajax()函数二次封装
         * @param url 请求路径
         * @param type 请求类型  post 增  delete 删除  put 修改  get 查
         * @param params 请求参数
         ...
         * @returns {string}
         */
        tableBatchCheck: function (table, field) {
            var checkStatus = table.checkStatus("tableId");
            var rows = checkStatus.data.length;
            if (rows > 0) {
                var idsStr = "";
                for (var i = 0; i < checkStatus.data.length; i++) {
                    idsStr += checkStatus.data[i][field] + ",";
                }
                return idsStr;
            } else {
        ...
        tableErrorMsg: function (content) {
            layer.msg(content, {icon: 2, time: 1000}, function () {
                // 刷新当前页table数据
                $(".layui-laypage-btn")[0].click();
            });
        },
